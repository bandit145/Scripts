...
foreach($name in $existing_servers){
	if($name -NotIn $data.name){
		...
		Remove-DnsServerResourceRecord -ZoneName meme.com -Name $name -ComputerName $dnsserver
		Remove-DhcpServerv4Reservation -ScopeID 192.168.1.0 -IPAddress $ipaddr.RecordData.IPv4Address.IPAddressToString -ComputerName $dhcpserver
	}
